...
$PC=$_POST['fn']; //name 
$AGN=$_POST['agn']; //Agency
$VAL=$_POST['val']; //Value of the Ticket
...
$s1="INSERT INTO `ticketing`(`pro_id`, `pers_id`, `agency_name`, `value`) VALUES ('$_POST[fln]','$x4','$_POST[agn]','$_POST[val]')";

$a=mysql_query($s1,$r);

echo "<script>alert('Done');window.location='ticketing.php';</script>";
...
<form name="ticketing" class="form" action="?ticketing" method="post">

    <div class="header">
    <center><h1>Ticketing</h1></center>
    </div>
...
<td><select name="fln" class="input" required id="sel1">
  <option>SELECT</option>
					...
					mysql_select_db("nomination",$r);
							...
							$s1="SELECT * FROM apo_sending WHERE pro_id NOT IN(SELECT pro_id FROM file_closing) GROUP by pro_id";
...
<select name="ti"  class="input4">
  ...
  <option>SELECT</option>
				...
				<?php
					...
					mysql_select_db("nomination",$r);
							...
							$z="SELECT * FROM nom_rec_c WHERE pro_id NOT IN (SELECT pro_id FROM file_closing) AND pers_id NOT IN (SELECT pers_id FROM ticketing) GROUP by name";
 ...
 <table>
 <tr>
<td  width="31.3%">Agency</td>
<td><input type="text" name="agn"  class="input" size="40" placeholder="" required></td>
</tr>

 <tr>
<td align = "left">Value of the Ticket (Rs.)</td>
<td><input type="text" name="val"  class="input" size="40" placeholder="" required></td>
</tr>
